...
rootpath = '../chinese_L-12_H-768_A-12'
config_path = os.path.join(rootpath, 'bert_config.json')
checkpoint_path = os.path.join(rootpath, 'bert_model.ckpt')
...
train_path = '../datasets/train_data.json'
dev_path = '../datasets/dev_data.json'
...
schema_path = '../datasets/all_50_schemas'
...
def index():
    with app.test_request_context():
        ...
        result = {"relation_extract": {"url": url_for("relation_extract"),
                                   "params": ["txt"]}}
...
@app.route("/ml/relation_extract/", methods=["GET"])
def relation_extract():
    ...
    txt = str(request_args.get("txt", -1))

    # 构建特征矩阵
    # vec = [[txt]]
    # print("vec: {0}".format(vec))

    print('txt', type(txt), txt)

    # 生成预测结果
    # predict_result = list(extract_items(txt))
    # result = ["林俊杰","歌手","可惜没如果"]
    # predict_result = int(model.predict(vec)[0])
    print('item', extract_items(str(txt)))
    predict_result = extract_items(txt)
    print("predict_result1: {0}".format(predict_result))
...
if __name__ == "__main__":
    ...
    app.run(port=8000, threaded=False)  #多线程问题,加threaded=False解决,或者在上一步加载模型后先运行一次extract_items("")