...
def make_model(InputSize):
    model=Sequential()
    model.add(Dense(units=128, kernel_initializer='normal', activation='sigmoid',input_shape=(InputSize,)))
...
#     print(trainY, testY)
    ...
    #训练模型并保存为module.h5
    model.fit(train_X,train_Y,batch_size=16,epochs=10,verbose=1,validation_data=(test_X,test_Y),shuffle=True)
    model.save_weights('modulef11.h5')

    #加载已保存的模型
    # model.load_weights('module1.h5',by_name=False)
